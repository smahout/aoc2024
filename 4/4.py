...
[(lines.append(grid.diagonal(-i)), lines.append(flip(grid.diagonal(-i)))) for i in
 range(0 - grid.shape[1], grid.shape[0])]  # \
[(lines.append(lr_grid.diagonal(-i)), lines.append(flip(lr_grid.diagonal(-i)))) for i in
 range(0 - grid.shape[1], grid.shape[0])]  # /
...
for row in range(1, grid.shape[1] - 1):
    for col in range(1, grid.shape[0] - 1):
        if grid[row][col] == 'A':
            ...
            xmas += 1 if (diagonal1 == ('M', 'S') or diagonal1 == ('S', 'M')) and (
                        diagonal2 == ('M', 'S') or diagonal2 == ('S', 'M')) else 0
print("Part 2: ", xmas)
